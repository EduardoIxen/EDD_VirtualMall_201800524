...
type allPedidos []*Pedidos.AutoGenerated
var lsPedidos = allPedidos{}
...
func cargartienda(w http.ResponseWriter, r *http.Request)  {
	...
	for _, dat := range listaDatos{
		...
		fmt.Println("Tiendas agregadas:", contador)
	}
	/*fmt.Println("///////////// Comienzo de vectores ////////////////")  //mostrar vectores con su contenido de tiendas
...
func cargarPedidos(W http.ResponseWriter, r *http.Request) {
	lsPedidos = allPedidos{}
	var nuevaEntrada *Pedidos.AutoGenerated
	reqBody, err := ioutil.ReadAll(r.Body)
	if err != nil{
		fmt.Println("-----------Error al cargar pedidos--------------")
	}
	json.Unmarshal(reqBody, &nuevaEntrada)
	lsPedidos = append(lsPedidos, &(*nuevaEntrada))

	for _, general := range lsPedidos{
		for _, contenido := range general.Pedidos{
			aux := &(*vector[obtenerPosicion(contenido.Departamento, contenido.Calificacion, contenido.Tienda)].Primero)
			sepa := strings.Split(contenido.Fecha,"-")
			for aux != nil {
				if aux.Tienda.Nombre == contenido.Tienda && aux.Tienda.Calificacion == contenido.Calificacion{
					anioPedido, _ := strconv.Atoi(sepa[2])
					mesPedido, _ := strconv.Atoi(sepa[1])
					diaPedido, _ := strconv.Atoi(sepa[0])
					fmt.Println(diaPedido)
					if aux.ArbolPedidos == nil{ //no cuenta con un arbol de pedidos
						nuevoArbPed := Pedidos.Nuevo_ArbolPedido()
						Pedidos.Insertar(&(*nuevoArbPed), anioPedido)
						aux.ArbolPedidos = &(*nuevoArbPed)
						fmt.Println("Insertado en -> ",obtenerPosicion(contenido.Departamento, contenido.Calificacion, contenido.Tienda))
						fmt.Println(anioPedido)
						fmt.Println(mesPedido)

						nuevaLs := Pedidos.New_ListaPedidos()
						Pedidos.InsertarPedido(mesPedido, &(*nuevaLs))
						Pedidos.Find(&(*nuevoArbPed), anioPedido).ListaPedidos = nuevaLs

						aux2 := Pedidos.BuscarNodoPedido(mesPedido, Pedidos.Find(&(*nuevoArbPed), anioPedido).ListaPedidos) //buscar nodo del mes del pedido
						if aux2 != nil{
							if aux2.MatrizDispersa == nil{
								aux2.MatrizDispersa = MatrizDispersa.New_MatrizDis(diaPedido, contenido.Departamento)
								colaPedido := MatrizDispersa.New_ColaPedido()
								lsProd := MatrizDispersa.New_ListaProductos()
								for _, codigoProd := range contenido.Productos{
									//fmt.Println("cod prod en linsta",codigoProd)
									MatrizDispersa.InsertarProductos(codigoProd.Codigo, lsProd)
								}
								MatrizDispersa.Push(&(*colaPedido), &(*lsProd))
								MatrizDispersa.InsertarEnMatriz(diaPedido, contenido.Departamento, colaPedido, &(*aux2.MatrizDispersa))
								fmt.Println("Inserte en el primer if matriz disp")
							}else{
								fmt.Println("NO CUENTA CON ARBOL PERO SI MATRIZ -.-")
							}
						}
					}else { //si cuenta con arbol de pedidos
						Pedidos.Insertar(&(*aux.ArbolPedidos), anioPedido)
						fmt.Println(aux.Tienda.Nombre)
						fmt.Println(aux.Tienda.Calificacion)
						fmt.Println("errore posi", Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).Anio)
						fmt.Println("errore posi", Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos)

						if Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos == nil { //no tiene lista de meses
							nuevaLs := Pedidos.New_ListaPedidos()
							Pedidos.InsertarPedido(mesPedido, &(*nuevaLs))
							Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos = &(*nuevaLs)

							aux2 := Pedidos.BuscarNodoPedido(mesPedido, Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos)
							if aux2 != nil{
								if aux2.MatrizDispersa == nil{
									Pedidos.BuscarNodoPedido(mesPedido, Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos).MatrizDispersa = MatrizDispersa.New_MatrizDis(diaPedido, contenido.Departamento)
									colaPedido := MatrizDispersa.New_ColaPedido()
									lsProd := MatrizDispersa.New_ListaProductos()
									for _, codigoProd := range contenido.Productos{
										//fmt.Println("cod prod en linsta",codigoProd)
									...
									MatrizDispersa.Push(&(*colaPedido), &(*lsProd))
									MatrizDispersa.InsertarEnMatriz(diaPedido, contenido.Departamento, &(*colaPedido), &(*aux2.MatrizDispersa))
									fmt.Println("Inserte en el segundo if matriz disp")
								}
							}
						}else { //ya tiene arbol y lista de meses
							fmt.Println("no es niiiil", diaPedido)
							Pedidos.InsertarPedido(mesPedido, Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos) //add mes

							lsProd := MatrizDispersa.New_ListaProductos()
							for _, cont := range contenido.Productos{
								//fmt.Println("cod prod en linsta ultimi else ",cont)
								MatrizDispersa.InsertarProductos(cont.Codigo, &(*lsProd))
							}
							fmt.Println()
							if Pedidos.BuscarNodoPedido(mesPedido, Pedidos.Find(aux.ArbolPedidos, anioPedido).ListaPedidos).MatrizDispersa != nil{ //tiene matriz
								aux2 := Pedidos.BuscarNodoPedido(mesPedido, Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos).MatrizDispersa
								fmt.Println("auxxx2 ",aux2)
								if MatrizDispersa.Find_NodoContenido(contenido.Departamento, diaPedido, Pedidos.BuscarNodoPedido(mesPedido, Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos).MatrizDispersa).ColaPedidos == nil{ //no tine cola de pedidos
									fmt.Println("add cola")
									colaPedido := MatrizDispersa.New_ColaPedido()
									MatrizDispersa.Push(&(*colaPedido), &(*lsProd))
									MatrizDispersa.InsertarEnMatriz(diaPedido, contenido.Departamento, colaPedido, Pedidos.BuscarNodoPedido(mesPedido, Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos).MatrizDispersa)
								}else{
									//aux3 := MatrizDispersa.Find_NodoContenido(contenido.Departamento, diaPedido, Pedidos.BuscarNodoPedido(mesPedido, Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos).MatrizDispersa).ColaPedidos
									MatrizDispersa.Push(&(*MatrizDispersa.Find_NodoContenido(contenido.Departamento, diaPedido, Pedidos.BuscarNodoPedido(mesPedido, Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos).MatrizDispersa).ColaPedidos),&(*lsProd))
									fmt.Println("Actualice cola pedidos en el ultimo else")
									//MatrizDispersa.InsertarEnMatriz(diaPedido, contenido.Departamento,&(*MatrizDispersa.Find_NodoContenido(contenido.Departamento, diaPedido, Pedidos.BuscarNodoPedido(mesPedido, Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos).MatrizDispersa).ColaPedidos), &*Pedidos.BuscarNodoPedido(mesPedido, Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos).MatrizDispersa)
								}
							}else{// no tiene matriz
								Pedidos.BuscarNodoPedido(mesPedido, Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos).MatrizDispersa = MatrizDispersa.New_MatrizDis(diaPedido, contenido.Departamento)
								colaPedido := MatrizDispersa.New_ColaPedido()
								MatrizDispersa.Push(&(*colaPedido), &(*lsProd))
								MatrizDispersa.InsertarEnMatriz(diaPedido, contenido.Departamento, &(*colaPedido), &(*Pedidos.BuscarNodoPedido(mesPedido, Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos).MatrizDispersa))
						...
						//Pedidos.InsertarPedido(mesPedido, Pedidos.Find(&(*aux.ArbolPedidos), anioPedido).ListaPedidos)
						//asignar correctamente el arbol y la lista de cada noso!!!!!!!!!!!!!!!!!!!!
						//fmt.Println("Insertado en -> ",obtenerPosicion(contenido.Departamento, contenido.Calificacion, contenido.Tienda))
					}
				}
				aux = aux.Siguiente
	...
	Pedidos.Generar_Grafo(vector[16].Primero.ArbolPedidos)
	Pedidos.ListarPedidoFecha(vector[16].Primero.ArbolPedidos.Raiz.ListaPedidos)
	fmt.Println("comienza")
	MatrizDispersa.ListarFilas(vector[16].Primero.ArbolPedidos.Raiz.ListaPedidos.Primero.MatrizDispersa)
	fmt.Println("comenza segundo")
	MatrizDispersa.ListarColumnas(vector[16].Primero.ArbolPedidos.Raiz.ListaPedidos.Primero.MatrizDispersa)
	MatrizDispersa.GraficarMatriz(vector[16].Primero.ArbolPedidos.Raiz.ListaPedidos.Primero.MatrizDispersa)
			...
			for aux != nil{
				if aux.Tienda.Nombre == inv.Tienda{
					if aux.Tienda.ArbolProd == nil{
						...
						Grafo.Generar_Grafo(arbolInv)
						//fmt.Scanln()
					}else {
						for _, prod := range inv.Productos{
							var pr = Arboles.NewProduct(prod.Nombre, prod.Codigo, prod.Descripcion, prod.Precio, prod.Cantidad, prod.Imagen)
							Arboles.Insert(&(*aux.Tienda.ArbolProd), &(*pr))
						}
						Grafo.Generar_Grafo(&(*aux.Tienda.ArbolProd))
						//fmt.Scanln()
					}
				}
				aux = aux.Siguiente
...
func matrizDispersa(w http.ResponseWriter, r *http.Request)  {
	lsPedidos = allPedidos{}
	var nuevaEntrada *Pedidos.AutoGenerated
	reqBody, err := ioutil.ReadAll(r.Body)
	if err != nil{
	...
	matriz := MatrizDispersa.New_Matriz2()
	for _, general := range lsPedidos {
		for _, contenido := range general.Pedidos {
			sepa := strings.Split(contenido.Fecha,"-")
			diaPedido, _ := strconv.Atoi(sepa[0])
			if matriz.Principal == nil{
				matriz = MatrizDispersa.Insertar_NodoPrincipal(diaPedido, contenido.Departamento)
				...
				MatrizDispersa.Push(&(*colaPedido), &(*lsProd))
				MatrizDispersa.InsertarEnMatriz(diaPedido, contenido.Departamento, &(*colaPedido), &(*matriz))
				fmt.Println("Primer if", diaPedido)
			}else{
				lsProd := MatrizDispersa.New_ListaProductos()
				for _, codigoProd := range contenido.Productos{
					MatrizDispersa.InsertarProductos(codigoProd.Codigo, &(*lsProd))
				}
				if MatrizDispersa.Find_NodoContenido(contenido.Departamento, diaPedido, &(*matriz)).ColaPedidos == nil{
					colaPedido := MatrizDispersa.New_ColaPedido()
					MatrizDispersa.Push(&(*colaPedido), &(*lsProd))
					MatrizDispersa.InsertarEnMatriz(diaPedido, contenido.Departamento, &(*colaPedido), &(*matriz))
					fmt.Println("ultimo if", diaPedido)
				}else{
					MatrizDispersa.Push(&*MatrizDispersa.Find_NodoContenido(contenido.Departamento, diaPedido, &(*matriz)).ColaPedidos,&(*lsProd))
					//MatrizDispersa.InsertarEnMatriz(diaPedido, contenido.Departamento, &(*MatrizDispersa.Find_NodoContenido(contenido.Departamento, diaPedido, &(*matriz)).ColaPedidos), &(*matriz))
					fmt.Println("ultimo else", diaPedido)
				}
			}
		}
	}
	MatrizDispersa.GraficarMatriz(matriz)
}
...
func main() {
	...
	router.HandleFunc("/matrizDispersa", matrizDispersa).Methods("POST")
